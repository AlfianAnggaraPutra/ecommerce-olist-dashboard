...
PALETTE  = [C_BLUE, C_RED, C_GREEN, C_ORANGE, C_PURPLE, C_GRAY]
...
def load_data():
    ...
    df = df.merge(df_customers[['customer_id', 'customer_state', 'customer_unique_id']],
                  on='customer_id', how='left')
    df = df.merge(df_payments[['order_id', 'payment_type', 'payment_value']],
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("Alfian Anggara Putra Afandy")
...
st.markdown("Analisis penjualan, kepuasan pelanggan, dan distribusi geografis platform Olist Brasil.")
st.markdown("---")
...
st.markdown('<p class="section-sub">Bagaimana tren total pendapatan bulanan Olist selama Januari 2017 – Agustus 2018?</p>', unsafe_allow_html=True)
...
with col_viz1:
    if len(monthly) > 0:
        ...
        axes[0].set_title('Total Pendapatan Bulanan — E-Commerce Olist', fontsize=13, fontweight='bold', pad=12)
        axes[0].set_xticks(range(len(monthly)))
        axes[0].set_xticklabels(monthly['order_month'], rotation=45, ha='right', fontsize=8)
        axes[0].legend(fontsize=9)
        style_ax(axes[0])

        # panel bawah — pertumbuhan
...
if len(monthly) > 0:
    st.markdown(
        ...
        f'sebesar <b>R$ {np_val:.2f}M</b>. Pertumbuhan pendapatan bersifat konsisten sepanjang 2017, '
        f'dengan pola musiman yang terlihat jelas menjelang akhir tahun.</div>',
        unsafe_allow_html=True
    )
...
# ── VIZ 2: KATEGORI & ULASAN ──────────────────────────────────────────────
st.markdown('<p class="section-title">⭐ Pertanyaan 2 — Skor Ulasan per Kategori Produk</p>', unsafe_allow_html=True)
st.markdown('<p class="section-sub">Kategori mana yang ulasannya paling baik dan paling buruk? Bagaimana hubungannya dengan harga?</p>', unsafe_allow_html=True)
...
if len(df_v2) > 0:
    ...
    with col_v2a:
        ...
        ax2.set_title('Skor Ulasan per Kategori Produk', fontsize=12, fontweight='bold')
        ax2.set_xlim(3, 5.4)
        style_ax(ax2)
    ...
    with col_v2b:
        ...
        ax3.set_title('Harga vs Skor Ulasan per Kategori\n(ukuran titik = jumlah ulasan)',
    ...
    st.markdown(
        ...
        f'Korelasi harga vs ulasan hanya <b>{kor:.3f}</b> — artinya harga bukan penentu utama kepuasan pelanggan.</div>',
        unsafe_allow_html=True
    )
...
# ── RFM ───────────────────────────────────────────────────────────────────
st.markdown('<p class="section-title">🎯 Analisis Lanjutan — RFM Segmentasi Pelanggan</p>', unsafe_allow_html=True)
st.markdown('<p class="section-sub">Mengelompokkan pelanggan berdasarkan Recency, Frequency, dan Monetary.</p>', unsafe_allow_html=True)
...
with col_r3:
    ...
    for segmen, warna in WARNA_SEGMEN.items():
        sub = df_rfm[df_rfm['segment'] == segmen]
        if len(sub) > 0:
            ax6.scatter(sub['Recency'], sub['Monetary'],
                        c=warna, alpha=0.45, s=12, label=segmen)
...
# ── GEOSPATIAL ─────────────────────────────────────────────────────────────
st.markdown('<p class="section-title">🗺️ Analisis Lanjutan — Distribusi Geografis</p>', unsafe_allow_html=True)
st.markdown('<p class="section-sub">Persebaran pesanan dan pendapatan per negara bagian di Brasil.</p>', unsafe_allow_html=True)
...
st.markdown(
    '<div class="insight-box">💡 <b>Insight:</b> Pesanan sangat terkonsentrasi di negara bagian '
    '<b>SP (São Paulo)</b> dan <b>RJ (Rio de Janeiro)</b>. Wilayah timur laut seperti CE, PE, dan BA '
    'masih memiliki potensi pertumbuhan yang besar untuk ekspansi logistik.</div>',
    unsafe_allow_html=True
)
...
tab1, tab2, tab3 = st.tabs(["Pertanyaan 1", "Pertanyaan 2", "Rekomendasi"])

with tab1:
    st.markdown("""
    Pendapatan bulanan Olist tumbuh secara konsisten sepanjang 2017 dan mencapai puncaknya di pertengahan 2018.
    Ada pola musiman yang terlihat, di mana pendapatan cenderung lebih tinggi menjelang akhir tahun.
    Pertumbuhan ini utamanya didorong oleh peningkatan volume pesanan, bukan kenaikan harga rata-rata produk.
    Bulan-bulan dengan pertumbuhan negatif umumnya terjadi setelah periode puncak (rebound effect).
    """)

with tab2:
    st.markdown("""
    Terdapat perbedaan yang jelas antar kategori produk dalam hal kepuasan pelanggan.
    Kategori seperti health_beauty dan bed_bath_table secara konsisten mendapat ulasan lebih tinggi.
    Kategori teknikal seperti computers_accessories dan telephony cenderung mendapat ulasan lebih rendah,
    kemungkinan karena ekspektasi pelanggan yang lebih tinggi terhadap produk-produk tersebut.
    Korelasi antara harga dan skor ulasan tergolong lemah, sehingga harga bukan faktor penentu utama kepuasan.
    """)

with tab3:
    st.markdown("""
    - Rencanakan kampanye promosi sebelum bulan-bulan yang secara historis memiliki pendapatan tinggi.
    - Lakukan evaluasi kualitas produk dan akurasi deskripsi pada kategori dengan ulasan rendah.
    - Terapkan program loyalitas eksklusif untuk segmen **Champions** dan strategi win-back untuk segmen **Lost**.
    - Perluas jangkauan logistik ke negara bagian timur laut Brasil yang masih memiliki penetrasi rendah.
    - Dorong penggunaan metode pembayaran cicilan untuk meningkatkan nilai rata-rata transaksi.
    """)

st.markdown("---")
st.markdown(
    "<center><small>Dashboard E-Commerce Olist · Alfian Anggara Putra Afandy · "
